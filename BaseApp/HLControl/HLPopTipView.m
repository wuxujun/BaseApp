...
#import "HLPopTipView.h"

@interface HLPopTipView()

@property (nonatomic,strong,readwrite)id    targetObject;

@end

@implementation HLPopTipView
@synthesize backgroundColor,delegate,message,customView,targetObject,textColor,textFont,textAlignment,animation,maxWidth,disableTapToDismiss;
...
- (void)presentPointingAtView:(UIView *)targetView inView:(UIView *)containerView animated:(BOOL)animated {
	...
	if (animated) {
        if (animation == HLPopTipAnimationSlide) {
            self.alpha = 0.0;
            CGRect startFrame = finalFrame;
            startFrame.origin.y += 10;
            self.frame = startFrame;
        }
		else if (animation == HLPopTipAnimationPop) {
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        ...
        self.animation = HLPopTipAnimationSlide;
    }
    return self;
}
