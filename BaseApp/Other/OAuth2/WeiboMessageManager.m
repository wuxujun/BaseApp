...
- (id)init {
    self = [super init];
    if (self) {
        requestManager = [[OAuthRequestManager alloc] initWithDelegate:self];
...
{
    return [requestManager getOAuthCodeUrl];
}
...
{
    [requestManager getPublicTimelineWithCount:count withPage:page];
}

//获取登陆用户的UID
-(void)getUserID
{
    [requestManager getUserID];
}

//获取任意一个用户的信息
-(void)getUserInfoWithUserID:(long long)uid
{
    [requestManager getUserInfoWithUserID:uid];
}

//根据微博消息ID返回某条微博消息的评论列表
-(void)getCommentListWithID:(long long)weiboID
{
    [requestManager getCommentListWithID:weiboID];
}
...
{
    [requestManager getBilateralIdListAll:uid sort:sort];
}

-(void)getBilateralIdList:(long long)uid count:(int)count page:(int)page sort:(int)sort
{
    [requestManager getBilateralIdList:uid count:count page:page sort:sort];
}
...
{
    [requestManager getBilateralUserListAll:uid sort:sort];
}

//获取用户的关注列表
-(void)getFollowingUserList:(long long)uid count:(int)count cursor:(int)cursor
{
    [requestManager getFollowingUserList:uid count:count cursor:cursor];
}

//获取用户粉丝列表
-(void)getFollowedUserList:(long long)uid count:(int)count cursor:(int)cursor
{
    [requestManager getFollowedUserList:uid count:count cursor:cursor];
}

//关注一个用户 by User ID
-(void)followByUserID:(long long)uid
{
    [requestManager followByUserID:uid];
}

//关注一个用户 by User Name
-(void)followByUserName:(NSString*)userName
{
    [requestManager followByUserName:userName];
}

//取消关注一个用户 by User ID
-(void)unfollowByUserID:(long long)uid
{
    [requestManager unfollowByUserID:uid];
}

//取消关注一个用户 by User Name
-(void)unfollowByUserName:(NSString*)userName
{
    [requestManager unfollowByUserName:userName];
}

//获取某话题下的微博消息
-(void)getTrendStatues:(NSString *)trendName
{
    [requestManager getTrendStatues:trendName];
}

//关注某话题
-(void)followTrend:(NSString*)trendName
{
    [requestManager followTrend:trendName];
}

//取消对某话题的关注
-(void)unfollowTrend:(long long)trendID
{
    [requestManager unfollowTrend:trendID];
}

//发布文字微博
-(void)postWithText:(NSString*)text
{
    [requestManager postWithText:text];
}

//发布文字图片微博
-(void)postWithText:(NSString *)text image:(UIImage*)image
{
    [requestManager postWithText:text image:image];
}
...
{
    [requestManager getHomeLine:sinceID maxID:maxID count:count page:page baseApp:baseApp feature:feature];
}
...
{
    [requestManager getUserStatusUserID:uid sinceID:sinceID maxID:maxID count:count page:page baseApp:baseApp feature:feature];
}

//转发一条微博
-(void)repost:(NSString*)weiboID content:(NSString*)content withComment:(int)isComment
{
    [requestManager repost:weiboID content:content withComment:isComment];
}

//按天返回热门微博转发榜的微博列表
-(void)getHotRepostDaily:(int)count
{
    [requestManager getHotRepostDaily:count];
}

//按天返回热门微博评论榜的微博列表
-(void)getHotCommnetDaily:(int)count
{
    [requestManager getHotCommnetDaily:count];
}

//获取某个用户的各种消息未读数
-(void)getUnreadCount:(NSString*)uid
{
    [requestManager getUnreadCount:uid];
}
